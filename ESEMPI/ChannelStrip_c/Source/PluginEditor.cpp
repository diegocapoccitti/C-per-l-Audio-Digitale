    ...
    : AudioProcessorEditor (&p), audioProcessor (p)
{
    ...
    outputGainKnob.setColour(juce::Slider::ColourIds::thumbColourId, juce::Colours::yellow);
...
void ChannelStripAudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    //Definisci il colore e la grandezza del font
    ...
    //Titolo bypass
    g.drawFittedText ("BYPASS", bypassButton.getX(), paralMixKnob.getBottom() + 40, bypassButton.getWidth(), 15, juce::Justification::centred, 1);
    //Titolo meters
    ...
    g.drawFittedText ("Q", qBellKnob.getX() + 30, qBellKnob.getY() + 35, qBellKnob.getWidth(), 10, juce::Justification::centred, 1);
    ...
    //Contorno finestra
    g.drawRoundedRectangle(getLocalBounds().toFloat(), 40.f, 2.f); 
}
...
void ChannelStripAudioProcessorEditor::resized()
{
    ...
    //Bypass
    bypassButton.setBounds(paralMixKnob.getX(), outputGainKnob.getY() - 10, dimKnobSmall, dimKnobSmall/2);
